...
*/
  ...
  * @requires jQuery
  */
Groupon.Touch = Groupon.Touch || {};
Groupon.Touch.Base = function() {
  var mainDiv = null;
  /**
  * Show Locations
  *
  ...
  * @returns
  */
  var showLocations = function(e){
    e.preventDefault();
    var divisions = new Groupon.Collections.Divisions()
      , $main = $('#main')
      , $locationsPage = $('#locations-div');

    if($locationsPage.length)
      Groupon.Touch.Base.animatePages($main, $locationsPage, 'slideup', false);

    else {
      $locationsPage = $(Mustache.to_html(Groupon.MustacheTemplates['touch/application/_division_drawer'], {loading: true}));
      $main.before($locationsPage);
      Groupon.Touch.Base.animatePages($main, $locationsPage, 'slideup', false);
      ...
      divisions.fetch({
        success: function() {
          $locationsPage
          .removeClass('loading')
          .html($(Mustache.to_html(Groupon.MustacheTemplates['touch/application/_division_drawer'], {divisions: divisions.models})).html());
        }
      });
    }

    $.mobile.currentPage = $locationsPage.data('animation', 'slideup');
  };
  /**
  * Trigger location change
  *
  * @method triggerLocationChange
  * @param {event} e
  * @returns
  */
  var handleLocTrigger = function(e){
    $('#locations-div').trigger('locationChange', [this, e]);
    e.preventDefault();
    return false;
  };
  /**
  * Check for orientaion
  *
  ...
  */
  ...
  /**
  * Handle tap on fieldset
  *
  * @method setInputFocus
  * @param
  * @returns
  */
  var setInputFocus = function(e){
    $("input", this).focus();
  };
  return{
    init: function(){
      ...
      if ($("#loc-menu") && $("#locations-div")){
        $("#loc-menu").bind("click", showLocations);
        $("#locations-div .back").live('click', this.hideLocations);
        $("#locations-div a").click(handleLocTrigger);
        mainDiv = $("#main");
      }
      $("body").bind('orientationchange', updateOrientation);
      updateOrientation();
      if (Groupon.TouchData != undefined){
        if (Groupon.TouchData.error != undefined){ alert(Groupon.TouchData.error); }
        if (Groupon.TouchData.success != undefined){ alert(Groupon.TouchData.success); }
      } 
      $(".apply-tooltip").blur(inputBlur);
      $(".apply-tooltip").focus(inputFocus);
      $.each($(".apply-tooltip"), inputBlur);
      //Android is throwing a hissy fit
      if (!$("body").hasClass('android')){
        $("fieldset.single-input").click(setInputFocus);
      }
    },
    /**
    * Animate Page
    *
    * @method animatePage
    * @param {element} fromPage
    * @param {element} toPage
    * @param {string} animation
    * @param {bool} backwards
    * @returns
    */
    animatePages: function(fromPage, toPage, animation, backwards) {
      var callback = function animationEnd(event){
        toPage.removeClass('in ' + animation);
        fromPage.removeClass('current ui-page-active out ' + animation);
        if (backwards) {                    
          toPage.removeClass('reverse');
      ...
      if (backwards) {
        toPage.addClass('reverse');
        fromPage.addClass('reverse');
      }
      toPage.addClass(animation + ' in current ui-page-active');
      fromPage.addClass(animation + ' out');
    },
    /**
    * Animate Div to Half Page height
    *
    * @method animateHalfPage
    * @param {HTML Element} halfPage
    * @param {string} animation style
    * @param {bool} toggle reverse animtion
    * @param {int} animate to this pixel
    * @returns
    */
    animateHalfPage: function(halfPage, animation, reverse, top){
      var callback = function animationEnd(event){
        halfPage.setStyle("top", top);
        if (reverse) { halfPage.removeClass('current reverse'); }
        halfPage.unbind('webkitAnimationEnd');
      };
      halfPage.one('webkitAnimationEnd', callback);
      if (reverse) { halfPage.addClass('reverse'); }
      else { halfPage.addClass(animation + " current"); 
      }
    },
    /**
    * Hide Locations
    ...
    * @returns
    */
    hideLocations: function(e){
      Groupon.Touch.Base.animatePages($("#locations-div"), mainDiv, "slideup", true);
    },
    /**
    * Show Progress
    *
    ...
    */
    showProgress: function(e){
      var submit = null;
      if (typeof (e.data) !== "undefined" && typeof (e.data.src) !== "undefined"){ submit = e.data.src; } 
      else { submit = $(":submit", this); }
    ...
    */
    setLocation: function(val){
      $("#locations-div .current").removeClass("current");
      $("#locations-div a[title='" + val + "']").addClass("current");
    }
  };
}();
Groupon.Touch.Base.init();
...
Groupon.Analytics = {
  ...
  trackEvent: function(eventType, params, optional) {
    ...
    optional = _.extend({ 'click': params[2], 'alt': params[1] }, optional);
  ...
  enablePDFTracking: function() {
    $('a[href$=pdf]').bind('click', function(event) {
      Groupon.Analytics.trackEvent('trackClick', ['DownloadPrepFlyer', this.pathname, location.pathname]);
    });
  },
...
$.behavior('Groupon.AnalyticsEventTracking', {
  _init: function() {
    this.trackOnClick = !$(this.element).hasClass('trackNow');
  ...
  track: function(eventType) {
    if (this.element.attr("data-google-analytics-tags") != undefined) {
      Groupon.Analytics.trackEvent(eventType, this.constructParams(this.element.attr("data-google-analytics-tags").split(" ")));
    }

    var thisBehavior = this;
    _.each($(this.element).attr('class').split(/\s/), function(classname) {
...
$(function() {
  $('.G_event').AnalyticsEventTracking();
});